...
import { domains, maintenanceWindows } from "@uni-proxy-manager/database/schema";
...
app.post("/domains/:domainId/enable", zValidator("json", enableMaintenanceSchema), async (c) => {
  const { domainId } = c.req.param();
  ...
  try {
    ...
    await db
      ...
      .set({
        ...
        maintenanceBypassIps: validatedBypassIps || domain.maintenanceBypassIps,
...
app.put("/domains/:domainId/bypass-ips", zValidator("json", updateBypassIpsSchema), async (c) => {
  const { domainId } = c.req.param();
  const data = c.req.valid("json");
...
app.get("/domains/:domainId/windows", async (c) => {
  ...
  try {
    ...
    const windowsList = await db.query.maintenanceWindows.findMany({
      ...
      orderBy: (maintenanceWindows, { desc }) => [desc(maintenanceWindows.createdAt)],
    });
...
app.get("/windows", async (c) => {
  ...
  try {
    ...
    if (domainId) {
      ...
      windows = await db.query.maintenanceWindows.findMany({
        where: active === "true" ? eq(maintenanceWindows.isActive, true) : undefined,
        with: {
          domain: true,
        },
        orderBy: (maintenanceWindows, { desc }) => [desc(maintenanceWindows.createdAt)],
...
app.post("/windows", zValidator("json", scheduleMaintenanceSchema), async (c) => {
  const data = c.req.valid("json");
...
async function queueHaproxyReload(reason: HaproxyReloadJobData["triggeredBy"], affectedDomainIds: string[]) {
  try {
